...
namespace MalbersAnimations.Selector
{
    ...
    {
#if ENABLE_INPUT_SYSTEM
        private MInputLink selectorMInputLink;
        private InputAction clickAction;
#endif

        protected virtual float DeltaTime => s_manager.DeltaTime;
...
#if ENABLE_INPUT_SYSTEM
            selectorMInputLink = GetComponentInParent<MInputLink>();

            if (selectorMInputLink != null)
                clickAction = selectorMInputLink.InputActions.FindAction("Click");
#endif
        }
        ...
        void Update()
        {
            ...
            ClickReleased(clickAction.WasReleasedThisFrame());
#endif
            SelectionAction();
            AnimateIdle();
        }
        ...
        protected virtual void AnimateIdle()
        {
            ...
            if (!IsChangingItem && !isAnimating && !IsSwapping) //
            {
                ...
                if (RotateIdle)
                {
                    CurrentItem.transform.Rotate(TurnTableVector, ItemRotationSpeed * DeltaTime * 10, Space.Self);
                }

                if (ScaleIdle && ScaleIdleAnim)
                {
                    IdleTimeScale += DeltaTime;
...
#if ENABLE_INPUT_SYSTEM
            CurrentEventData.position = Pointer.current.position.ReadValue();
#else
        		CurrentEventData.position = Input.mousePosition;            //Send to the Current Event Data the Mouse Position
#endif
...
#if ENABLE_INPUT_SYSTEM
                    Vector2 NormMousePos = new Vector2(Pointer.current.position.ReadValue().x / Screen.width, Pointer.current.position.ReadValue().y / Screen.height);
#else
        				Vector2 NormMousePos = new Vector2(Input.mousePosition.x / Screen.width, Input.mousePosition.y / Screen.height);
#endif
...
#if ENABLE_INPUT_SYSTEM
                DeltaMousePos = (Vector2)MouseStartPosition - Pointer.current.position.ReadValue();
#else
        			DeltaMousePos = (Vector2)(MouseStartPosition - Input.mousePosition);	//Store the amount of distance travelled sice the first click/touch
#endif
...
#if ENABLE_INPUT_SYSTEM
                        if ((Pointer.current.position.ReadValue().x < center.x && dragHorizontal)
                        || ((Pointer.current.position.ReadValue().y < center.y && !dragHorizontal)))
#else
							if ((Input.mousePosition.x < center.x && dragHorizontal)
							|| ((Input.mousePosition.y < center.y && !dragHorizontal)))
#endif
        ...
        protected IEnumerator PlayTransformAnimation(MItem item, TransformAnimation animTransform)
        {
            if (item != null && animTransform != null)
            {
                ...
                {
                    if (s_manager.ignoreTimeScale)
                    {
                        yield return new WaitForSecondsRealtime(animTransform.delay);         //Wait for the Delay     
                    }
                    else
                    {
